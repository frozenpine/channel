...
func TestFileStore(t *testing.T) {
	...
	store := chanio.NewFileStore(flowFile)

	if err := store.Open(os.O_WRONLY | os.O_CREATE | os.O_TRUNC); err != nil {
		t.Fatal("store open failed:", err)
	}

	t1 := chanio.RegisterType(&Int{}, func() chanio.PersistentData {
		return &Int{}
	})
	t2 := chanio.RegisterType(&Varaint{}, func() chanio.PersistentData {
		return &Varaint{}
	})
...
func TestRegisterType(t *testing.T) {
	var (
		count   = 5
		tidList = make([]chanio.TID, count)
	)

	for i := 0; i < count; i++ {
		tidList[i] = chanio.RegisterType(&Varaint{}, func() chanio.PersistentData {
			return &Varaint{}
		})
	}
...
func BenchmarkFileStoreWR(b *testing.B) {
	tid := chanio.RegisterType(&Varaint{}, func() chanio.PersistentData {
		return &Varaint{}
	})

	store := chanio.NewFileStore("flow_bench.dat")
...
func BenchmarkFileStoreRD(b *testing.B) {
	chanio.RegisterType(&Varaint{}, func() chanio.PersistentData {
		return &Varaint{}
	})
